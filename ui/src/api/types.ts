...
export class GameCopier {

    public static from_json({ running, appid, peerurl, dest }: any): GameCopier {
        return new GameCopier(running, appid, peerurl, dest)
    }
    public constructor(
...
export class Peer {
    ...
    public async loadGames(): Promise<void> {
        try {
            const res: Response = await this.apiGet("http://localhost:9771/games")

            this.games = (await res.json() as any[]).map((game) =>
                SteamGame.from_json(game),
            )
        } catch (err) {
    ...
    public async loadHeaderImage(appID: SteamGame["appID"]): Promise<string | undefined> {
        try {
            const res: Response = await this.apiGet(`http://localhost:9771/games/${appID}/images/header?encode=base64`)
